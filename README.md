...
Перед первым запуском необходимо запусть скрипт models.py для создания базы данных.
Сервер запускается из скрипта server.py на хосте 127.0.0.1 и порте 8000.
Клиент запускается из скрипта client.py.
...
1. Получить все непрочитанные сообщения, если пользователь уже существует или получить последние 20 сообщений, если пользователь новый (пользователь создается автоматически).
